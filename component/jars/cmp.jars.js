...
class ComponentJars {
	...
	static html() {
		...
		return `<div class="${ this.cmpTitle }">
			...
			<div id="${ this.cmpTitle }_galery"></div>
			<div>
				<table class="${ this.cmpTitle }-tableJars">
					...
					<thead>
					<tbody id="${ this.cmpTitle }_tbodyJars"><tbody>
				</table>
			</div>
		</div>`;
	}
	...
	static clc( elem ) {
		...
		let divGalery = document.getElementById( this.cmpTitle + '_galery' );
		let htmlGalery = '';
		divGalery.innerHTML = htmlGalery;

		let divTBody = document.getElementById( this.cmpTitle + '_tbodyJars' );
		let tbody = '';
		divTBody.innerHTML = tbody;
		...
		arrFiltered.forEach( ( k, i ) => {
			...
			htmlGalery += `<div class="${ this.cmpTitle }-each">
				<img src="img/jars/${ k.id }.jpg" title="" alt="">
				<div class="${ this.cmpTitle }-each-num">${ n }</div>
